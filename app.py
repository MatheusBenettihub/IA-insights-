...
def get_indicators():
    ...
    # CoinGecko com key — 4 janelas de 365 dias cobrindo 4 anos
    ...
    # Busca 4 períodos anuais do mais antigo para o mais recente
    periods = [
        (now_ts - 4*365*86400, now_ts - 3*365*86400),
        (now_ts - 3*365*86400, now_ts - 2*365*86400),
        (now_ts - 2*365*86400, now_ts - 1*365*86400),
        (now_ts - 1*365*86400, now_ts),
    ]

    for from_ts, to_ts in periods:
        try:
            cg_params = {
                "vs_currency": "usd",
                "from": from_ts,
            ...
            r = requests.get(
                ...
                params=cg_params, headers=HEADERS, timeout=20
            )
            if r.status_code == 200:
                d = r.json()
                prices = d.get("prices", [])
                vols   = d.get("total_volumes", [])
                # Filtra para pegar só 1 ponto por dia (evita duplicatas)
                seen_days = set()
                for p in prices:
                    day = _dt.utcfromtimestamp(p[0]/1000).strftime("%Y-%m-%d")
                    if day not in seen_days:
                        seen_days.add(day)
                        all_prices.append(p[1])
                for v in vols:
                    day = _dt.utcfromtimestamp(v[0]/1000).strftime("%Y-%m-%d")
                    all_vols.append(v[1])
        except Exception as e:
    ...
    if len(all_prices) > 100:
        closes_d = all_prices
        vols_d   = all_vols[:len(closes_d)]

    # Fallback: Binance simples
    if len(closes_d) < 50:
        try:
            r = requests.get(
